...
- **`FB_API_KEY`, `FB_DATABASE_URL`, `HARVEST_SCALE_DEVICE_SECRET`**: the Firebase project's public API key, the database's URL, and this individual scale's one-time bootstrap secret described in section 5. The secret must be generated and registered on the backend before a new scale unit can connect at all, following the steps documented directly above these constants in the `.ino` file.

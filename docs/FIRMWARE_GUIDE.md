...
**Taring.** Taring means telling the scale "whatever the platform reads right now is zero," so that only the weight of what gets placed on top of it counts. After warm-up, the firmware averages 30 raw readings (`TARE_SAMPLES`) and stores that average as the zero-offset. This must happen with the platform empty of any harvest item, which is why warm-up and taring both print reminders to keep the platform clear — the platform/tray itself stays on the load cell the whole time, so tare zeroes it out along with everything else in one step. An earlier version tried to separately compensate for the tray's own weight (a `PLATFORM_TARE_GRAMS` constant, folded into the tare offset by hand), on the assumption the tray got placed on *after* taring — but the tray was actually always present during tare, so that extra subtraction double-counted it and pinned every reading at a constant -50 g. Removed; plain `tare()` already handles it.
...
**Display value.** The LCD shows `reportedGrams` — the raw per-cycle weight (`get_units(READING_SAMPLES)`, already averaged over 10 samples) until the stability buffer described in section 3 has collected a full set of real samples, and the plain average of that buffer from then on (`stabilityAverageGrams()`). Ordinary cycle-to-cycle HX711 noise (a few tenths of a gram on this load cell) meant the same physical object could show a slightly different number moment to moment, or a different number on the LCD than what actually got uploaded a moment later — averaging real recent samples removes that once something has actually been sitting long enough to have a buffer's worth of readings. There used to be a second, EMA-smoothed `displayWeightGrams` value that eased 25% of the way toward each new reading every cycle instead, but that made placing an item on the platform look like the display was "counting" up to the real value rather than just showing it — `displayWeightGrams` still exists and is set to `reportedGrams` each cycle, so the "Uploading..."/"Saved:" messages and the normal weighing screen always show the exact same number, shown to 1 decimal place rather than rounded to a whole gram. It stops updating altogether once a load is confirmed and uploaded (see the note at the end of this section and in section 5), so the screen holds on the "Saved: ..." message instead of drifting with noise on an unchanged load.

## 3. How the scale decides a weight is "final" and uploads it

The firmware does not upload every reading. It waits until it is confident someone has placed an item and it has settled, then uploads once automatically. This works in three stages, using the raw (per-cycle) weight to decide *when* it's stable:
...
Once all three conditions are met, the firmware uploads the stability buffer's average weight and sets an `uploadedThisLoad` flag so it will not upload again for the same item, even if the reading keeps sitting there stable.
...
- **Live weight.** The current weight is written to `devices/{deviceId}/harvestScale/liveWeight` every 5 seconds. This just overwrites the same single value each time, so it does not pile up records. It is throttled to once every 5 seconds (rather than every 500ms reading) because Firebase network calls are slow on this chip, and calling them that often would overload it and can cause crashes. It stops updating (freezes at whatever it last showed) once a load has been confirmed and logged, so it doesn't keep wiggling with ordinary sensor noise next to a number that's already been finalized — see the "resetting after a change without full removal" note in section 3 for when it starts updating again. The LCD is frozen on the same schedule, for the same reason: once a load is confirmed, the screen holds on the "Saved: {weight} g" message it showed at upload time instead of continuing to refresh with new readings.

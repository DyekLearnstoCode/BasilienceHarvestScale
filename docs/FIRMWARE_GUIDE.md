...
**Display value.** The LCD shows the same raw, per-cycle weight (`get_units(READING_SAMPLES)`, already averaged over 10 samples) that the upload-stability logic evaluates — there used to be a second, EMA-smoothed `displayWeightGrams` value that eased 25% of the way toward each new reading every cycle to hide ordinary HX711 flicker, but that made placing an item on the platform look like the display was "counting" up to the real value instead of just showing it. `displayWeightGrams` still exists, but now it is simply set equal to the raw reading each cycle — it exists so the "Uploading..."/"Saved:" messages and the normal weighing screen always show the exact same number. It stops updating altogether once a load is confirmed and uploaded (see the note at the end of this section and in section 5), so the screen holds on the "Saved: ..." message instead of drifting with noise on an unchanged load.
...
- **`PLATFORM_TARE_GRAMS`** (50.0): the weight, in grams, of the empty weighing platform/tray itself. "Platform must be EMPTY. Taring..." means the bare load cell with no tray on it — the tray is placed on afterward, so the boot-time tare (see section 2) never sees its weight. This amount is folded into the tare offset itself exactly once, in `setup()` right after `tare()` succeeds — not subtracted per-reading in `loop()` — because doing it per-reading meant that if the tray happened to already be on the load cell during tare (so `tare()` had already zeroed its weight on its own), it got subtracted a second time, pinning every reading at a constant -50 g. If the physical platform/tray is ever swapped for a different one, re-weigh it empty and update this constant to match.
...
- **`LoadCellManager.h` / `LoadCellManager.cpp`**: wraps the third-party HX711 library. Handles safely starting up the HX711 chip without hanging forever if it is not actually connected, taking raw and averaged readings, taring (and directly adjusting the resulting tare offset, via `setOffset()`, to fold in `PLATFORM_TARE_GRAMS` — see section 7), storing the calibration factor, and converting raw readings into a weight in grams.
